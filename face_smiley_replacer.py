...
def load_with_exif(path):
    """
    Loads an image and applies EXIF orientation, returning a BGR numpy array.
    Requires Pillow; if not available, falls back to cv2.imread.
    """
    try:
...
def replace_faces_with_smileys_dnn(
    ...
    conf_threshold=0.85,
    face_scale=1.25,          # overall scale applied to the aspect-ratio-preserving fit
    ...
    boxes, confs = detect_faces_dnn_on_original(img_orig, net, conf_threshold)
    ...
    for (x, y, w, h), c in zip(boxes, confs):
        ...
        smiley = cv2.imread(smiley_path, cv2.IMREAD_UNCHANGED)
        if smiley is None:
            continue
        ...
        out = overlay_png(out, smiley_resized, place_x, place_y)

    return out

if __name__ == "__main__":

    img_orig = load_with_exif("input.png")
    if img_orig is None:
        raise FileNotFoundError(f"Konnte Bild nicht laden")
    ...
    result = replace_faces_with_smileys_dnn(
        ...
        model_path="models/res10_300x300_ssd_iter_140000.caffemodel",
        conf_threshold=0.85,
        face_scale=1.20,  # tweak: 1.0 for exact face size, >1 to cover more
        random_seed=datetime.now().timestamp(),
        print_detections=True
    )
